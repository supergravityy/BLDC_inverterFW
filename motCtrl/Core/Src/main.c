 ...
 * 
 * teleplot에 string이 뜨지 않음 -> teleplot에는 ">이름:값\n" 형식의 문자열이 와야 파싱해서 그래프로 띄워줌
 *
 * rpm 계산로직이 동작하다 정지해도 0으로 떨어지지 않고 이상한 값을 뱉음 -> LPF(IIR) 필터링된 rpm으로 new==old를 비교하니 절대 성립이 안되었던 것 
 *
 * PI 제어시, 지령값을 준 후, 스로틀을 당기지 않다가 스로틀을 당기면, 일정 시간동안 최대 출력을 내서 RPM이 지령값에 도달하지 못함
 * 									->  1ms마다 적분 term이 계속 누적되다가 줄어들 때까지 최고 출력을 유지하는 와인드업 현상
 *
 ...
 * slope = 7, Pi = 3.4, Ki = ?
 * 
 * 피딩을 안하면 몇초 후에 꺼지는 지를 테스트 중, 워치독 5~6초안에 꺼져야 하는데 약 32초후에 꺼짐 -> 초기화를 하고 reload를 해줘야 첫스타트부터 반영됨
 *
 * teraterm으로 sysinput을 검증하였지만, 한글자 칠때마다 테라텀에 글자가 안보임 -> FW가 에코모드 처럼 동작하게 하는 로직 필요
 *
 * >t를 하면 모터가 덜덜 떠는 현상이 있었음 -> isr 로직에서 mute가 안되어서 전류가 미친듯이 공급되었던 것
 *
 * 오버런 카운트가 1ms 태스크에서 지속적으로 발생 -> ADC와 UART의 폴링방식으로 인한 태스크 점유시간 높음 (ex:isr) -> 차후 개선점
 * --------------------------------------*/
 ...
 * uart.c에서 쓸모없는 함수 삭제
 * task 딜레이를 고려해 위상차를 준 100ms 태스크 새로 만듬 + uart 통신 문자열이 ">이름:값\n" 형식을 따르도록 수정
 * hallsens_check_zeroSpd을 빠른 응답성을 위해 tim isr로 옮김 + 필터링된 rpm이 아닌 필터링 직전의 rpm으로 old=new를 수행
 ...
 * 보드레잇을 115200으로 상승시킴
 * 워치독 초기화 로직에서 마지막에 첫스타트부터 reload값이 반영되게 리로드를 해주는 로직을 추가
 * 초기화 완료 및 전원 off 시 메시지 출력
 * uart로 input을 받을 수 있는 모드 (utils.h) 추가 -> 선택가능
 * uart rx 인터럽트를 이용한 기능 추가 (MCAL) -> sysinput 상위객체 기능추가
 * isr과 스케줄러에 이를 적용
 * Rx 인터럽트에서 FW가 에코모드 처럼 동작하게 하는 로직 추가
 * isr에서 에러,스로틀,제어일시정지 를 한번에 검사하게 수정함 -> 만족 안하면 mute
 * --------------------------------------*/
