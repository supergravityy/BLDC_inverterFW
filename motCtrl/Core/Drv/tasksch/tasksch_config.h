...
#define TASKSCH_CYCLE_MEASURING             (TASKSCH_CYCLE_MEASURE_DISABLE)      // NOTE : USER DEFINE
...
#define TASKSCH_MAJOR_CYCLE_HOOK            (TASKSCH_MAJOR_CYCLE_HOOK_DISABLE)   // NOTE : USER DEFINE
...
// INFO : LSI frequency can have an error of up to ±20%, so take the count with a margin of error.
    ...
    #define TASKSCH_WATCHDOG_TIMEOUT_CNT    (782)

    // Derived Macros -> Do not modify
    #define TASKSCH_WATCHDOG_1COUNT_MS      (TASKSCH_WATCHDOG_PRESCALER / TASKSCH_WATCHDOG_CLK_FREQ_HZ) // 256 / 32000 = 8ms
    #define TASKSCH_WATCHDOG_TIMEOUT_MS     (TASKSCH_WATCHDOG_TIMEOUT_CNT * TASKSCH_WATCHDOG_1COUNT_MS) // 782 * 6.4ms = 6.256s
#endif
